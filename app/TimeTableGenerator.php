...
class TimeTableGenerator
{
    ...
    private function slotsAreEnough()
    {
        ...
        foreach ($this->gene as $gene) {
            $totalUnits += $gene["hours_per_week"];
        }

        return $totalUnits <= $totalSlots - 10;
    }
    ...
    private function makeChromosomes()
    {
        $chromosomes = [];
        foreach ($this->gene as $gene) {
            for ($x = 1; $x <= $gene["hours_per_week"]; $x++) {
                $chromosomes[] = $gene["course_code"];
            }
        }
        ...
        for ($x = 0; $x < $size; $x++) {
            ...
            for ($y = 0; $y < $this->daysOfWeek; $y++) {
                for ($z = 0; $z < $this->sizeOfDay; $z++) {
                    ...
                    if (
                        $this->hasLevelWideCourses() &&
                        $this->levelWideCourses[$row][$col] != "-"
                    ) {
                    ...
                    if (isset($this->chromosomes[$x + 1])) {
                        if (
                            $this->chromosomes[$x + 1] ===
                                $this->chromosomes[$x] &&
                            !$this->hasUsedDoublePeriods($this->chromosomes[$x])
                        ) {
                            $this->timeTable[$row][$col + 1] =
                                $this->chromosomes[$x];
